module.exports = {
    ...
    execute: async (sock, m, args, settings) => {
        ...
        const { isGroup, isAdmin, isBotAdmin, isOwner, botJid, normalizeJid } = m;
        ...
        if (target === botJid) return reply('Cannot remove admin rights from the bot.');

        try {
            await sock.groupParticipantsUpdate(m.from, [target], 'demote');
            reply('Admin rights removed successfully.');
        } catch (err) {
